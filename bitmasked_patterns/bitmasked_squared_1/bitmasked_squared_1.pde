...
void draw() {
  ...
  for (int x = 0; x < xMax; x++) {
    for (int y = 0; y < yMax; y++) {
      ...
      result = b * b & bitmask;  // Bitmask, and use the result as the color of pixels
...
void keyPressed() {
  if (key == CODED) {
    ...
    if (key == '1') {
      ...
      String s = String.format("your_output/bitmasked_squared_1_%s_bitmask_%d_xOffset_%d_yOffset_%d_s%s.png", bitwiseString, bitmask, xOffset, yOffset, stamp());  // For distinction a stamp is used
